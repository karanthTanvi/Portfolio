...
export default function Work() {
  return (
    <section id="work" className="work-section">
      <style>{`
        /* TEMP: single featured project until the other case studies are done.
           To restore the grid: swap max-width/margin for the grid rules below. */
        .work-section {
          padding: 8rem 4rem;
          max-width: 860px;
          margin: 0 auto;
        }
        /* ORIGINAL GRID (uncomment when you have more projects):
        .work-section {
          padding: 8rem 4rem;
          display: grid;
        ...
        */
        ...
        @media (max-width: 900px) {
          .work-section { padding: 5rem 2rem; }
        }
      ...
      {/* TEMP: render only the first project */}
      {projects.slice(0, 1).map((project, index) => (
        <ProjectCard key={project.id} project={project} index={index} />
      ))}

      <p className="work-note">More case studies coming soon.</p>
    </section>
  )
}